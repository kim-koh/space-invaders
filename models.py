...
class Ship(GImage):
    ...
    def __init__(self,theme):
        ...
        if theme == 6:
            imagesource = 'santa.png'
        super().__init__(height=SHIP_HEIGHT,width=SHIP_WIDTH,bottom=SHIP_BOTTOM,x=GAME_WIDTH/2,source=imagesource)
    
    # METHODS TO MOVE THE SHIP AND CHECK FOR COLLISIONS
    def collides(self,bolt):
        """
        Returns: True of the bolt was fired by the player and collides with the ship.
        ...
        collision = False
        for item in bolt:
            a = (item.right,item.top)
            b = (item.right,item.bottom)
            c = (item.left,item.top)
            d = (item.left,item.bottom)
            if self.contains(a) or self.contains(b) or self.contains(c) or self.contains(d):
                if item._isPlayerBolt == False:
                    collision = True
                    bolt.remove(item)
        return collision
...
class Alien(GImage):
    ...
    def __init__(self,alientype,xpos,ypos,theme):
        """write a specification"""
        assert isinstance(alientype,str) and (alientype =='a' or alientype == 'b' or alientype =='c'), 'type must be a, b or c'
        #ASSERT THE X AND Y
        imagesource= ALIEN_IMAGES
        color = ['white','blue','red','purple','orange','green',None]
        if theme == 6:
            imagesource = XMAS_IMAGES
        if alientype == 'a':
            super().__init__(x=xpos,y=ypos, width=ALIEN_WIDTH, height=ALIEN_HEIGHT,source=imagesource[0],fillcolor=color[theme])
        if alientype == 'b':
            super().__init__(x=xpos,y=ypos, width=ALIEN_WIDTH, height=ALIEN_HEIGHT,source=imagesource[1],fillcolor=color[theme],)
        if alientype == 'c':
            super().__init__(x=xpos,y=ypos, width=ALIEN_WIDTH, height=ALIEN_HEIGHT,source=imagesource[2],fillcolor=color[theme],)  
        
    # METHOD TO CHECK FOR COLLISION (IF DESIRED)
    def collides(self,bolt):
        """
        Returns: True of the bolt was fired by the player and collides with this alien.
...
class Bolt(GRectangle):
    ...
    LIST MORE ATTRIBUTES (AND THEIR INVARIANTS) HERE IF NECESSARY
        _isPlayerBolt: True if bolt from player False if from Alien
    """
    
    # GETTERS AND SETTERS (ONLY ADD IF YOU NEED THEM)
    
    # INITIALIZER TO SET THE VELOCITY
    def __init__(self,xpos,ypos,isPlayerBolt):
        """spec here"""
        if isPlayerBolt:
            self._velocity = BOLT_SPEED
            self._isPlayerBolt = True
            super().__init__(x=xpos,y=ypos,fillcolor='red',linecolor='yellow',height=BOLT_HEIGHT,width=BOLT_WIDTH)
        else:
            self._velocity = -BOLT_SPEED
            self._isPlayerBolt = False
            super().__init__(x=xpos,y=ypos,fillcolor='cyan',linecolor='blue',height=BOLT_HEIGHT,width=BOLT_WIDTH)
    # ADD MORE METHODS (PROPERLY SPECIFIED) AS NECESSARY
